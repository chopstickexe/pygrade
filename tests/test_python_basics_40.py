import numpy as np
from pygrade import python_basics_40


def test_one():
    A = np.array(range(18, 34))
    assert np.array_equal(
        python_basics_40.solution_one(A), np.array([range(18 + i, 18 + i + 7) for i in range(10)])
    )
...
def test_two():
    ...
    assert (
        python_basics_40.solution_two(A)
...
def test_three():
    ...
    assert (python_basics_40.solution_three(A) == "Name, Night, Work, Problem, Age, Road, Company, Head")


def test_four():
    A = np.array(range(0, 25)).reshape(5, 5)
    assert np.array_equal(python_basics_40.solution_four(A), np.array([[0, 5, 10, 15, 20]]))


def test_five():
    A = np.array([[3, 10, 2, 11, 9, 17], [18, 11, 5, 16, 13, 12], [6, 5, 1, 14, 11, 17], [7, 1, 7, 19, 8, 13], [5, 16, 18, 1, 13, 5], [19, 2, 9, 10, 1, 19]])
    assert np.array_equal(python_basics_40.solution_five(A), np.array([[0, 0, 0, 11, 0, 17], [18, 11, 0, 16, 13, 12], [0, 0, 0, 14, 11, 17], [0, 0, 0, 19, 0, 13], [0, 16, 18, 0, 13, 0], [19, 0, 0, 0, 0, 19]]))
